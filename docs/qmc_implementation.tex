...
$N=4096$ and 211\,MB at $N=32768$ ($L=32$ in 3D). At low temperature a further
contribution comes from the worldline pools, which are sized from $\beta N$
because DSQSS preallocates them and treats an overflow as a successful exit;
these add $\sim130\,$MB at $L=32$, $\beta=8$, for $\sim320\,$MB total. A silent
calibration phase precedes sampling, its cost scaling with $\beta N$ --- seconds
at $L=16$, minutes at $L=32$ --- which the run flags so it is not mistaken for a
hang.
