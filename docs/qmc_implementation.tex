...
\subsection{Longitudinal channel: the structure factor}

DSQSS has no real-space $S^zS^z$ correlator. It measures the $k$-resolved
dynamic structure factor $S^{zz}(\mathbf{k},\tau)$ by Fourier-transforming the
instantaneous magnetization on each time slice and correlating slices. Real
space is recovered in the converter by an inverse transform,
\begin{equation}
  C^{zz}(\mathbf{r},\tau)
  ...
  \label{eq:zzft}
\end{equation}
DSQSS's own generator emits only the product set
$k_d\in\{0,\dots,L_d/2\}$, relying on
$S^{zz}(\mathbf{k})=S^{zz}(-\mathbf{k})$ to supply the rest. As
Section~\ref{sec:corr4} explains, that reasoning fails beyond one dimension, so
the code here measures the \emph{full} zone instead. Every $\mathbf{k}$ then
enters \eqref{eq:zzft} with weight one and no multiplicity bookkeeping is
required. Since distinct $\mathbf{k}$ are independent accumulators, statistical
errors propagate through \eqref{eq:zzft} in quadrature with the same weights
squared.
...
|-- patches/
|   `-- cf_antisymmetric.patch    adds the C^xy channel to the engine
|-- qmc/
...
is obtained by choosing one site per distance class. Sites within a class are
...
\paragraph{The fix.} \code{parse.write\_full\_bz\_wavevectors} overwrites the
\code{wv.xml} produced by \code{dla\_pre} with one covering every
$\mathbf{k}$ in the zone, before \code{dla} runs. The inverse transform is then
...
\begin{align}
  ...
  \mathrm{Im}\,C^{xy}(\mathbf{r},\tau) &= \tfrac12\, s_{\mathbf r}\,
      ...
      \qquad C^{yx}=-C^{xy}, \\
  C^{zz}(\mathbf{r},\tau) &= \frac1N\sum_{\mathbf k}
      S^{zz}(\mathbf{k},\tau)\cos(\mathbf{k}\!\cdot\!\mathbf{r}),
\end{align}
the last sum running over the full Brillouin zone with uniform weight.
...
\code{patches/cf\_antisymmetric.patch} (about 50 lines against DSQSS 2.0.6) adds
